...
typedef void (*FuncCallAddElement)(const char *pageId, const char *parentRef, const char *domStr, int domLen, const char *index_cstr);
...
typedef void (*FuncCallDIspatchMessage)(const char *clientId, const char *data, int dataLength, const char *callback, const char *vmId);
...
typedef void (*FuncOnReceivedResult)(long callback_id, std::unique_ptr<WeexJSResult>& result);
typedef void (*FuncUpdateComponentData)(const char* page_id, const char* cid, const char* json_data);


typedef bool (*FuncLog)(int level, const char *tag,
                        const char *file,
                        unsigned long line,
                        const char *log);
