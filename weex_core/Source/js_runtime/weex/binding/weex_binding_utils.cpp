...
unicorn::ScopeValues WeexBindingUtils::atob(
    WeexGlobalObjectV2 *nativeObject,
...
unicorn::ScopeValues WeexBindingUtils::btoa(
    WeexGlobalObjectV2 *nativeObject,
    const std::vector<unicorn::ScopeValues> &vars) {
  std::string origin_data;
  if (vars.size() > 0 && vars[0]->IsString()) {
...
unicorn::ScopeValues WeexBindingUtils::clearNativeTimeout(
    WeexGlobalObjectV2 *nativeObject,
    std::vector<unicorn::ScopeValues> &vars) {
  if (nullptr == nativeObject) {
    return unicorn::RuntimeValues::MakeInt(0);
...
unicorn::ScopeValues WeexBindingUtils::callT3DLinkNative(
    WeexGlobalObjectV2 *nativeObject,
    const std::vector<unicorn::ScopeValues> &vars) {
  if (nullptr == nativeObject) {
...
unicorn::ScopeValues WeexBindingUtils::callGCanvasLinkNative(
    WeexGlobalObjectV2 *nativeObject,
    const std::vector<unicorn::ScopeValues> &vars) {
  if (nullptr == nativeObject) {
    return unicorn::RuntimeValues::MakeString(std::string(""));
...
unicorn::ScopeValues WeexBindingUtils::__updateComponentData(
    WeexGlobalObjectV2 *nativeObject,
    const std::vector<unicorn::ScopeValues> &vars) {
  std::string page_id;
  std::string cid;
