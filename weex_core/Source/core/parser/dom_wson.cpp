 ...
 * */
RenderObject* parserWson2RenderObject(
    wson_parser& parser,
    RenderObject* parent,
    int index,
...
RenderObject *parserWson2RenderObjectNew(
    wson_parser& parser, RenderObject *parent, int index, const std::string &pageId,bool reserveStyles) {
  int objectType = parser.nextType();
  if (!parser.isMap(objectType)) {
    parser.skipValue(objectType);
    return nullptr;
...
static void WsonGenerate(
    wson_parser& parser,
    const std::string& parentRef,
    int index,
  const WsonObjectGenerator& genObject) {
  int objectType = parser.nextType();
  if (!parser.isMap(objectType)) {
    parser.skipValue(objectType);
    return;
