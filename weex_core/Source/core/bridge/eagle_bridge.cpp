...
EagleRenderObject EagleBridge::WeexCoreHandler::GetEagleRenderObject(
...
int EagleBridge::WeexCoreHandler::RefreshFinish(const char* page_id, const char* task, const char* callback) {
  WeexCore::WeexCoreManager::Instance()->getPlatformBridge()->core_side()->ForceLayout(page_id);
  return WeexCore::WeexCoreManager::Instance()->getPlatformBridge()->platform_side()->RefreshFinish(page_id, task, callback);
}
    
std::unique_ptr<ValueWithType> EagleBridge::WeexCoreHandler::CallNativeModule(
    const char* page_id,
    const char* module,
    const char* method,
    ...
    const char* options,
    int options_length) {
  return WeexCoreManager::Instance()->getPlatformBridge()->platform_side()->CallNativeModule(
      page_id, module, method, arguments, arguments_length, options, options_length);
}

#if OS_IOS
std::unique_ptr<ValueWithType> EagleBridge::WeexCoreHandler::RegisterPluginModule(
    const std::string &name, const std::string &class_name, const std::string &version) {
  return WeexCoreManager::Instance()->getPlatformBridge()->platform_side()->RegisterPluginModule(name.c_str(), class_name.c_str(), version.c_str());
}

std::unique_ptr<ValueWithType> EagleBridge::WeexCoreHandler::RegisterPluginComponent(
    const std::string &name, const std::string &class_name, const std::string &version) {
  return WeexCoreManager::Instance()->getPlatformBridge()->platform_side()->RegisterPluginComponent(name.c_str(), class_name.c_str(), version.c_str());
}
...
void EagleBridge::WeexCoreHandler::CallNativeComponent(
    const char* page_id,
    const char* module,
    const char* method,
    const char* arguments,
    ...
    const char* options,
    int options_length) {
  WeexCoreManager::Instance()->getPlatformBridge()->platform_side()->CallNativeComponent(page_id, module, method, arguments, arguments_length, options, options_length);
}
...
bool EagleBridge::WeexCoreHandler::RemoveRenderObject(
    const std::string &page_id, const std::string &ref) {
  return RenderManager::GetInstance()->RemoveRenderObject(page_id, ref);
}
    
bool EagleBridge::WeexCoreHandler::AddRenderObject(
    const std::string &page_id, const std::string &parent_ref, int index, EagleRenderObject root) {
  return RenderManager::GetInstance()->AddRenderObject(page_id, parent_ref, index, root.render_object_impl_);
}
    
bool EagleBridge::WeexCoreHandler::MoveRenderObject(
    const std::string &page_id, const std::string &ref,const std::string &parent_ref, int index) {
  return RenderManager::GetInstance()->MoveRenderObject(page_id, ref, parent_ref, index);
}
    
bool EagleBridge::WeexCoreHandler::RemoveEvent(
    const std::string &page_id, const std::string &ref, const std::string &event) {
  return RenderManager::GetInstance()->RemoveEvent(page_id, ref, event);
}
    
bool EagleBridge::WeexCoreHandler::AddEvent(
    const std::string &page_id, const std::string &ref, const std::string &event) {
  return RenderManager::GetInstance()->AddEvent(page_id, ref, event);
}
    
bool EagleBridge::WeexCoreHandler::UpdateAttr(
    const std::string &page_id, const std::string &ref, const char *data) {
...
bool EagleBridge::WeexCoreHandler::UpdateStyle(
    const std::string &page_id, const std::string &ref, const char *data) {
...
int EagleBridge::WeexCoreHandler::ExecJS(
