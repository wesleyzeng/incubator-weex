...
private:
  ...
  virtual float CalculateFreeSpaceAlongMainAxis(
      const float &width, const float &height, const float &currentLength) const {
    float ret;
  ...
  inline float calcItemSizeAlongAxis(
      const WXCoreLayoutNode* const node, const bool horizontal, const bool useHypotheticalSize = false) const {
    float ret;
    if (horizontal) {
  ...
  inline void limitMainSizeForFlexGrow(
      WXCoreFlexLine* const flexLine, const Index childIndex, const float flexGrow) {
  ...
  void determineMainSize(float width, float height);
  void determineCrossSize(float, float, bool);
  void determineCrossSize(float, float, WXCoreFlexLine *);
