...
std::tuple<bool, float, float> WXCoreLayoutNode::calculateBFCDimension(
    const std::pair<float,float>& renderPageSize) {
...
std::pair<bool,float> WXCoreLayoutNode::calculateBFCWidth(float width, const float renderPageWidth) {
  bool sizeChanged = false;
  if (isnan(width) && mParent != nullptr &&
...
Index WXCoreLayoutNode::expandItemsInFlexLine(
    WXCoreFlexLine* const flexLine, const float maxMainSize, const Index startIndex) {
...
void WXCoreLayoutNode::layout(float left, float top, float right, float bottom,
                              ...
                              const std::pair<float,float>* const renderPageSize) {
  if (absoluteFlexItem) {
...
void WXCoreLayoutNode::calcAbsoluteOffset(
    float &left, float &top, float &right, float &bottom, const std::pair<float,float>* const renderPageSize) {
...
void WXCoreLayoutNode::layoutFlexlineHorizontal(
    const float width,
    const WXCoreFlexLine *const flexLine,
    float &childLeft,
    float &childRight,
...
void WXCoreLayoutNode::layoutFlexlineVertical(
    const float height,
    const WXCoreFlexLine *const flexLine,
    float &childTop,
    float &childBottom,
