...
bool RenderPageCustom::CreateBody(
...
// TODO: Optimize this.
bool RenderPageCustom::AddRenderObject(
    const std::string& ref, const std::string& type,
...
bool RenderPageCustom::MoveRenderObject(
    const std::string &ref, const std::string &parent_ref, int index) {
  if (target_) {
...
bool RenderPageCustom::UpdateStyle(
    const std::string &ref, std::vector<std::pair<std::string, std::string>> *styles) {
  if (target_) {
...
// TODO: Optimize this.
bool RenderPageCustom::UpdateAttr(
    const std::string &ref, std::vector<std::pair<std::string, std::string>> *attrs) {
  if (target_) {
...
std::unique_ptr<ValueWithType> RenderPageCustom::CallNativeModule(
    const char *module,
    const char *method,
    const char *argumentsWson,
    int arguments_length,
    const char *optionsWson,
    int options_length) {
  ...
  if (target_) {
    if (target_->shouldHandleModuleMethod(module, method)) {
      ...
      auto result = target_->callNativeModule(
          page_id_, module, method,
    ...
    return RenderPageBase::CallNativeModule(
        module, method, argumentsWson, arguments_length, optionsWson, options_length);
  }
...
void RenderPageCustom::CallNativeComponent(
    const char* ref,
    const char* method,
    const char* argumentsWson,
    int arguments_length,
...
// Life cycle
void RenderPageCustom::SetDefaultHeightAndWidthIntoRootRender(
